...
$(document).ready(function () {
  ...
  $("#ParkDropDown").on("change", function () {
    ...
    $.ajax({
      ...
      var weatherURL =
        "https://api.weatherapi.com/v1/forecast.json?key=ae859661fd494bcea6740915202602&q=" +
        parkZip +
        "&days=3";
      $.ajax({
        url: weatherURL,
        ...
        $(".weatherTemp0").text(
          response.forecast.forecastday[0].day.avgtemp_f + "F"
        );
        $(".weatherTemp1").text(
          response.forecast.forecastday[1].day.avgtemp_f + "F"
        );
        $(".weatherTemp2").text(
          response.forecast.forecastday[2].day.avgtemp_f + "F"
        );
